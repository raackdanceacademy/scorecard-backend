...
try:
    ...
    print(f"⚠️ Could not initialize branches: {e}")
...
# ============================================================
# FULL MAIN.PY LOGIC (Filled from your original code)
# ============================================================
...
def _score_dict(score):
    if not score:
        return {"financial_health": 0, "student_growth": 0, "operations_discipline": 0, "brand_quality": 0, "customer_experience": 0, "local_marketing": 0, "base_score": 0, "penalty": 0, "final_score": 0, "rating": "No Data", "action": "No data submitted"}
    return {
...
def submit_it_form(payload: ITSubmissionIn, db: Session = Depends(get_db)):
    ...
    is_new = False
    if not it_sub:
        is_new = True; it_sub = ITSubmission(branch_id=branch.id, report_month=report_month); db.add(it_sub)
    ...
    it_sub.complaints_received = payload.complaints_received; it_sub.complaints_resolved = payload.complaints_resolved
    it_sub.referrals = payload.referrals; it_sub.event_satisfaction = payload.event_satisfaction

    it_sub.revenue_target = payload.revenue_target; it_sub.revenue_actual = payload.revenue_actual
    it_sub.tshirt_due = payload.tshirt_due; it_sub.tshirt_paid = payload.tshirt_paid
    it_sub.salary_due = payload.salary_due; it_sub.salary_paid = payload.salary_paid
    it_sub.hotel_due = payload.hotel_due; it_sub.hotel_paid = payload.hotel_paid
    it_sub.merch_due = payload.merch_due; it_sub.merch_paid = payload.merch_paid
    it_sub.fee_collection_rate = payload.fee_collection_rate; it_sub.operating_margin = payload.operating_margin

    it_sub.opening_students = payload.opening_students; it_sub.new_student_target = payload.new_student_target
    it_sub.new_enrollments = payload.new_enrollments; it_sub.dropouts = payload.dropouts; it_sub.closing_students = payload.closing_students

    it_sub.marketing_activities = payload.marketing_activities; it_sub.partnerships = payload.partnerships
    it_sub.unauthorized_discount = payload.unauthorized_discount; it_sub.false_reporting = payload.false_reporting
    it_sub.trainer_misconduct = payload.trainer_misconduct; it_sub.comments = payload.comments
...
def _recalculate_score(db: Session, branch_id: int, report_month: date):
    ...
    if not score_row:
        score_row = MonthlyScore(branch_id=branch_id, report_month=report_month); db.add(score_row)
